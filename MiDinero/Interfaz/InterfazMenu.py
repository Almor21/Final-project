...
def GuardarArchivos(Base: Principal) -> None:
    fichero = open("../Archivos/Base", "wb")
    pickle.dump(Base, fichero)
    fichero.close()


def CargarArchivos() -> Principal:
    if os.path.exists("../Archivos/Base"):
        fichero = open("../Archivos/Base", "rb")
